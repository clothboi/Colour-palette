...
function resolveIdealMixGuidance(targetLab, request, fallbackRecipeEntries, fallbackCandidatePaints) {
  ...
  if (singlePaintMatch && singlePaintMatch.distance <= IDEAL_SINGLE_PAINT_DISTANCE_THRESHOLD) {
    const missingEntries = hasOwnedEquivalent(singlePaintMatch.paint, state.ownedPaints)
      ? []
      : [{ paint: singlePaintMatch.paint, massPercent: 100 }];
  ...
  function getCardLines(entry) {
    ...
    if (entry.catalog_matches?.length) {
      ...
      entry.catalog_matches.forEach((match) => {
        lines.push(`${match.color_name} - Item ${match.item_number} - Series ${match.series || "n/a"} - ${match.reason}`);
      });
    }
