...
const EXPORT_GRADIENT_SATURATION_MIN = 0.1;
const EXPORT_GRADIENT_SATURATION_MAX = 2.5;
...
const renderProfile = (() => {
  ...
  return {
    ...
    // Gradient export: iOS Safari renders canvas filter blur heavier — compensate
    gradientBlurBoost: isIOSDevice ? 0.82 : 1,
    gradientSaturateBoost: isIOSDevice ? 108 : 118,
  };
})();
...
function getMergedSaveGradientNodes() {
  ...
  return state.saveExport.gradientNodes.map((node) => {
    ...
    return {
      ...
      influence: clamp(typeof node.influence === "number" ? node.influence : 0.5, EXPORT_GRADIENT_INFLUENCE_MIN, EXPORT_GRADIENT_INFLUENCE_MAX),
      saturation: clamp(typeof node.saturation === "number" ? node.saturation : 1, EXPORT_GRADIENT_SATURATION_MIN, EXPORT_GRADIENT_SATURATION_MAX),
...
function getExportWheelRingFill(color, ringIndex) {
  ...
  const saturationScale = ringIndex === 2
    ? 0.8
    : ringIndex === 1
      ? 0.6
      : 0.4;
  const adjusted = getAdjustedExportLchRgb(color, {
    lightnessShift: ringIndex === 2 ? 10 : ringIndex === 1 ? 20 : 30,
    chromaScale: saturationScale,
  });
  return rgbToHex(adjusted.r, adjusted.g, adjusted.b);
}

function getSortedWheelColors() {
  return [...state.colors];
}
...
function getGradientInfluenceDisplayRadius(percent, totalPercent, minDimension, influence = 1) {
  return getGradientBlobRadiusForDimension(percent, totalPercent, minDimension, influence) * 0.5;
}
...
function buildWheelExportBaseCanvas() {
  const orderedColors = getSortedWheelColors();
  const exportCanvas = document.createElement("canvas");
  const size = 1400;
  const center = size / 2;
  const radius = 600;
  const hollowFraction = 0.2;
  const ringFractions = [0.14, 0.18, 0.22, 0.26];
...
function buildGradientExportBaseCanvas(options) {
  ...
  // Gaussian RBF field — compute at 1/4 resolution then scale up on composite.
  // Each pixel's colour is the Gaussian-weighted average of all node colours;
  // alpha is driven by the strongest single-node weight so areas with no nearby
  // node fade to transparent rather than blending to a muddy average.
  const blobScale = 4;
  const bw = Math.ceil(width / blobScale);
  const bh = Math.ceil(height / blobScale);
  const blobCanvas = document.createElement("canvas");
  blobCanvas.width = bw;
  blobCanvas.height = bh;
  const blobCtx = blobCanvas.getContext("2d");
  const blobMinDim = Math.min(bw, bh);

  const gaussNodes = nodes.map((node) => {
    const sigma = getGradientBlobRadiusForDimension(node.percent || 0, totalPercent, blobMinDim, node.influence);
    const sat = node.saturation ?? 1;
    const grey = (node.r + node.g + node.b) / 3;
    return {
      x: node.x * bw,
      y: node.y * bh,
      r: clamp(grey + (node.r - grey) * sat, 0, 255),
      g: clamp(grey + (node.g - grey) * sat, 0, 255),
      b: clamp(grey + (node.b - grey) * sat, 0, 255),
      twoSigmaSq: 2 * sigma * sigma,
    };
  });

  const imageData = blobCtx.createImageData(bw, bh);
  ...
  for (let py = 0; py < bh; py++) {
    for (let px = 0; px < bw; px++) {
      ...
      for (let i = 0; i < gaussNodes.length; i++) {
        const n = gaussNodes[i];
        const dx = px - n.x;
        const dy = py - n.y;
        const w = Math.exp(-(dx * dx + dy * dy) / n.twoSigmaSq);
        totalWeight += w;
      ...
      data[idx + 3] = Math.min(255, Math.sqrt(maxWeight) * 255);
    }
  }
  ...
  exportCtx.filter = `blur(${Math.max(28, Math.round(minDimension * 0.06 * renderProfile.gradientBlurBoost))}px) saturate(${renderProfile.gradientSaturateBoost}%)`;
  exportCtx.drawImage(blobCanvas, 0, 0, width, height);
  exportCtx.restore();
...
function renderSavePreviewOverlay() {
  ...
  savePreviewOverlay.style.touchAction = "none";
  ...
  if (activeNode) {
    const influenceRadius = getGradientInfluenceDisplayRadius(
    ...
    // Inner saturation ring
    const saturationRadius = clamp(influenceRadius * 0.35 * activeNode.saturation, 14, influenceRadius * 0.85);
    const satRing = document.createElement("div");
    satRing.className = "save-preview-saturation-ring";
    satRing.style.left = `${centerX}px`;
    satRing.style.top = `${centerY}px`;
    satRing.style.width = `${saturationRadius * 2}px`;
    satRing.style.height = `${saturationRadius * 2}px`;
    fragment.appendChild(satRing);

    const satHitSvg = document.createElementNS(svgNS, "svg");
    satHitSvg.style.position = "absolute";
    satHitSvg.style.left = `${centerX}px`;
    satHitSvg.style.top = `${centerY}px`;
    satHitSvg.style.transform = "translate(-50%, -50%)";
    satHitSvg.style.overflow = "visible";
    satHitSvg.style.pointerEvents = "none";
    satHitSvg.setAttribute("width", saturationRadius * 2);
    satHitSvg.setAttribute("height", saturationRadius * 2);
    const satHitCircle = document.createElementNS(svgNS, "circle");
    satHitCircle.setAttribute("cx", saturationRadius);
    satHitCircle.setAttribute("cy", saturationRadius);
    satHitCircle.setAttribute("r", saturationRadius);
    satHitCircle.setAttribute("fill", "none");
    satHitCircle.setAttribute("stroke", "transparent");
    satHitCircle.setAttribute("stroke-width", "20");
    satHitCircle.style.pointerEvents = "stroke";
    satHitCircle.style.cursor = "ns-resize";
    satHitCircle.addEventListener("pointerdown", (event) => {
      startSaveGradientSaturationDrag(event, activeNode.id);
    });
    satHitSvg.appendChild(satHitCircle);
    fragment.appendChild(satHitSvg);

    const satHandle = document.createElement("button");
    satHandle.className = "save-preview-saturation-handle";
    satHandle.type = "button";
    satHandle.dataset.nodeId = activeNode.id;
    satHandle.setAttribute("aria-label", `${activeNode.hex} saturation`);
    satHandle.title = `${activeNode.hex} saturation`;
    satHandle.style.left = `${centerX}px`;
    satHandle.style.top = `${centerY - saturationRadius}px`;
    satHandle.addEventListener("pointerdown", (event) => {
      startSaveGradientSaturationDrag(event, activeNode.id);
    });
    fragment.appendChild(satHandle);
  }
...
function updateSaveGradientNodeInfluenceFromClientPosition(clientX, clientY) {
  ...
  const baseRadius = getGradientInfluenceDisplayRadius(node.percent || 0, totalPercent, Math.min(bounds.width, bounds.height), 1);
...
function updateSaveGradientNodeSaturationFromClientPosition(clientX, clientY) {
  const bounds = getSavePreviewOverlayBounds();
  if (!bounds || !state.saveExport.gradientDragId) {
    return;
  }
  ...
  const totalPercent = Math.max(1, state.colors.reduce((sum, color) => sum + (color.percent || 0), 0));
  const influenceDisplayRadius = getGradientInfluenceDisplayRadius(node.percent || 0, totalPercent, Math.min(bounds.width, bounds.height), node.influence ?? 0.5);
  const baseRadius = influenceDisplayRadius * 0.35;
  const centerX = bounds.left + (clamp(node.x, 0, 1) * bounds.width);
  const centerY = bounds.top + (clamp(node.y, 0, 1) * bounds.height);
  const distance = Math.hypot(clientX - centerX, clientY - centerY);
  node.saturation = clamp(distance / Math.max(1, baseRadius), EXPORT_GRADIENT_SATURATION_MIN, EXPORT_GRADIENT_SATURATION_MAX);
}

function startSaveGradientSaturationDrag(event, id) {
  if (state.saveExport.layout !== EXPORT_LAYOUT_GRADIENT || saveModal.classList.contains("hidden")) {
    return;
  }
  ...
  state.saveExport.gradientActiveId = id;
  state.saveExport.gradientDragId = id;
  state.saveExport.gradientDragMode = "saturation";
  state.saveExport.gradientDragPointerId = event.pointerId ?? null;
  lockPageScroll(SCROLL_LOCK_SAVE_GRADIENT_DRAG);
  updateSaveGradientNodeSaturationFromClientPosition(event.clientX, event.clientY);
  scheduleSavePreviewRender();
}
...
function handleSaveGradientNodeDrag(event) {
  ...
  if (state.saveExport.gradientDragMode === "influence") {
    updateSaveGradientNodeInfluenceFromClientPosition(event.clientX, event.clientY);
  } else if (state.saveExport.gradientDragMode === "saturation") {
    updateSaveGradientNodeSaturationFromClientPosition(event.clientX, event.clientY);
  } else {
    updateSaveGradientNodeFromClientPosition(event.clientX, event.clientY);
  }
