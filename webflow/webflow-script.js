  ...
  function getCardLines(entry) {
    ...
    if (entry.catalog_matches?.length) {
      ...
      entry.catalog_matches.forEach((match) => {
        lines.push(`${match.color_name} - ${formatPaintDetailLine(match)} - ${match.reason}`);
      });
    }
